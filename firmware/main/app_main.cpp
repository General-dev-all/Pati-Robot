...
void guc_kipi_uygula(pati::GucKaynagi kaynak)
{
    ...
    // 🔴 BURADAKI DEGER YALNIZCA ILK ANLAR ICIN.
    //
    // Gercek deger NVS'te ve `ayar_baslat()` cok daha sonra kosuyor
    // (bu dosyanin sonlarinda). Arka isik ise gozler acilirken
    // yakiliyor, yani ARADA. O aralikta ne yazarsa o gorunuyor.
    //
    // ⚠️ BU YUZDEN MUHAFAZAKAR: %25. Buraya %100 yazmak, ebeveyn
    // %5 secmis olsa bile her acilista kisa bir TAM PARLAK cakma
    // demekti — karanlik bir odada uyandirici bir sey.
...
extern "C" void app_main()
{
    ...
    // Ekran parlakligi da ayni sebeple burada: yukaridaki cagri yalnizca
    // acilisin ilk anlarini kapsiyordu, ebeveynin sectigi deger ancak
    // simdi biliniyor.
    pati::ekran_parlaklik_ayarla(
        static_cast<float>(pati::ayar_parlaklik()) / 100.0f);
