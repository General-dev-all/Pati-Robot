...
export class TarayiciSes {
  constructor(gonderPcm) {
    ...
    // CALMA HIZI (tizlik) — masaustu Hoparlor'un yaptigi isin aynisi.
    //
    // Onceden bu mod tizlik ayarini TAMAMEN YOK SAYIYORDU: panelden
    // 1.30x secilse bile tarayici 24 kHz sabit caliyordu. Telefondan
    // baglanan biri ayarladigi seyi duymuyordu.
    //
    // Cozum bedava: 24 kHz veriyi daha yuksek orneklemeyle calmak hem
    // hizlandiriyor hem tizlestiriyor. Fizik geregi ayrilmiyorlar —
    // panelde de boyle yaziyor.
    this.hiz = 1.0;
  }
  ...
  cal(arrayBuffer) {
    ...
    // Ornekleme hizini `hiz` kadar yukseltmek = daha hizli + daha tiz.
    // Tarayici yeniden orneklemeyi kendi yapiyor.
    const oran = Math.max(0.5, Math.min(2.0, this.hiz || 1));
    const tampon = this.cikisCtx.createBuffer(1, pcm.length,
                                              Math.round(CIKIS_HZ * oran));
  ...
  /**
   * Sekme arka plana atilip geri gelince mikrofonu diriltir.
   *
   * NEDEN GEREKTI (30.07.2026, gercek kullanim): telefonda Pati
   * acikken baska bir uygulamaya gecip geri donunce Pati bir daha
   * uyanmiyordu. Sebep tarayici: sekme arka plandayken AudioContext
   * ASKIYA ALINIYOR, `onaudioprocess` durmus oluyor ve mikrofondan
   * tek bayt gitmiyor. Python tarafi sessizlik goruyor — uykudaki
   * Pati'yi uyandiracak ses hic ulasmiyor.
   *
   * Bazen daha kotusu oluyor: isletim sistemi mikrofonu geri aliyor
   * ve akis "ended" durumuna dusuyor. O zaman resume() yetmiyor,
   * izni yeniden almak gerekiyor.
   *
   * Tek care Durdur/Baslat idi (kapat() + ac() yapiyor). Bu fonksiyon
   * ayni isi oturumu kapatmadan yapiyor.
   *
   * ⚠ BU BIR TARAYICI SORUNU, ROBOTUN SORUNU DEGIL. ESP32'de tarayici
   *   yok; mikrofon I2S'ten kesintisiz akiyor ve boyle bir askiya
   *   alma kavrami bulunmuyor. Burasi sadece telefondan test yolunu
   *   ayakta tutuyor.
   *
   * Doner: bir sey yapildiysa true.
   */
  async tazele() {
    if (!this.acik) return false;
    let degisti = false;

    // 1. Askiya alinmis ses baglamlarini uyandir.
    for (const c of [this.girisCtx, this.cikisCtx]) {
      if (c && c.state === 'suspended') {
        try { await c.resume(); degisti = true; } catch { /* olur */ }
      }
    }

    // 2. Mikrofon akisi tamamen dustuyse izni yeniden al.
    const iz = this.akis && this.akis.getAudioTracks()[0];
    if (!iz || iz.readyState === 'ended') {
      this.kapat();
      await this.ac();          // izin hatirlaniyorsa sessizce aciliyor
      degisti = true;
    }
    return degisti;
  }
